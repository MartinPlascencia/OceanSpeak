...
export default class Background extends Phaser.GameObjects.Container {
    ...
    private createMenu(scene: Phaser.Scene, screen: Screen) {
        ...
        const titleText = scene.add.bitmapText(0, -backgroundFrame.displayHeight * 0.32, 'grobold_color', 'OCEAN SPEAK', 70).setOrigin(0.5).setLetterSpacing(15);
    ...
    private setNumberOfFishes(value: number) {
        this._numberOfFishes = value;
        this._numberFishesSliderText.setText('Number of Fishes : ' + value);
    }

    private setFishesSpeed(value: number) {
        this._fishesSpeed = value;
        this._speedFishesSliderText.setText('Speed : ' + value);
    }

    private createSliders(scene: Phaser.Scene, frame : Phaser.GameObjects.Image) {

        const fishesSlider = new Slider(scene, this._frame, {x: 0, y: -frame.displayHeight * 0.03}, frame.displayWidth * 0.6, frame.displayHeight * 0.1, 
            this._gameConfig.min_fishes_number, this._gameConfig.max_fishes_number, this.setNumberOfFishes.bind(this));
        
        this._numberFishesSliderText = scene.add.bitmapText(0, fishesSlider.y - frame.displayHeight * 0.14, 'grobold', 'Number of Fishes : ', 45).setOrigin(0.5);
        this._frame.add(this._numberFishesSliderText);

        const speedSlider = new Slider(scene, this._frame, {x: 0, y: fishesSlider.y + frame.displayHeight * 0.3}, frame.displayWidth * 0.6, 
            frame.displayHeight * 0.1, this._gameConfig.min_fish_speed, this._gameConfig.max_fish_speed, this.setFishesSpeed.bind(this));

        const speedSliderText = scene.add.bitmapText(0, speedSlider.y - frame.displayHeight * 0.14, 'grobold', 'Speed : ', 45).setOrigin(0.5);
